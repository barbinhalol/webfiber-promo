...
window.WF3D = {
  ...
  init(){
    ...
    const texSky=()=>{ const c=document.createElement('canvas'); c.width=16;c.height=256; const x=c.getContext('2d'); const g=x.createLinearGradient(0,0,0,256); g.addColorStop(0,'#1463cc'); g.addColorStop(.5,'#50a0f2'); g.addColorStop(1,'#c8e6ff'); x.fillStyle=g; x.fillRect(0,0,16,256); const t=new THREE.CanvasTexture(c); t.colorSpace=THREE.SRGBColorSpace; return t; };
    scene.background=texSky();
    scene.fog=new THREE.Fog(0xc8e6ff,28,54);
    ...
    // ---- CENÁRIO: arquibancada com torcida + placas WebFiber ----
    const texTorcida=()=>{ const c=document.createElement('canvas'); c.width=320;c.height=120; const x=c.getContext('2d');
      const g=x.createLinearGradient(0,0,0,120); g.addColorStop(0,'#091f42'); g.addColorStop(1,'#14356a'); x.fillStyle=g; x.fillRect(0,0,320,120);
      const cores=['#ffd23a','#ff3b7f','#2ee6c9','#ffffff','#3d8bff','#ff7a1a','#43d854'];
      for(let i=0;i<2200;i++){ x.fillStyle=cores[(Math.random()*cores.length)|0]; x.globalAlpha=0.45+Math.random()*0.55; x.fillRect(Math.random()*320, 14+Math.random()*104, 2.3, 2.3); }
      const t=new THREE.CanvasTexture(c); t.colorSpace=THREE.SRGBColorSpace; return t; };
    const arq=new THREE.Mesh(new THREE.PlaneGeometry(58,5.4), new THREE.MeshBasicMaterial({map:texTorcida(),fog:true}));
    arq.position.set(0,3.0,GOLZ-6); scene.add(arq);
    // bandeiras de escanteio
    const mkBandeira=(x,cor)=>{ const g2=new THREE.Group(); const mastro=new THREE.Mesh(new THREE.CylinderGeometry(0.05,0.05,1.7,8), M(0xdddddd,.5)); mastro.position.y=0.85; mastro.castShadow=true; g2.add(mastro); const ban=new THREE.Mesh(new THREE.PlaneGeometry(0.55,0.36), new THREE.MeshStandardMaterial({color:cor,side:THREE.DoubleSide,roughness:.7})); ban.position.set(0.28,1.5,0); g2.add(ban); g2.position.set(x,0,GOLZ+1); scene.add(g2); return ban; };
    mkBandeira(-GW/2-1.4,0xFFC400); mkBandeira(GW/2+1.4,0xFFC400);
    // placas de publicidade na linha de fundo (atrás do gol)
    const texPlaca=(txt,bg,fg)=>{ const c=document.createElement('canvas'); c.width=256;c.height=64; const x=c.getContext('2d'); x.fillStyle=bg; x.fillRect(0,0,256,64); x.fillStyle=fg; x.font='900 34px Arial,sans-serif'; x.textAlign='center'; x.textBaseline='middle'; x.fillText(txt,128,37); const t=new THREE.CanvasTexture(c); t.colorSpace=THREE.SRGBColorSpace; return t; };
    const placas=[['WEBFIBER','#1565E0','#ffffff'],['850 MEGA','#FFC400','#114433'],['FIBRA','#7B3FF2','#ffffff'],['1 GIGA','#0a3a86','#FFC400']];
    for(let i=0;i<7;i++){ const p=placas[i%placas.length]; const pl=new THREE.Mesh(new THREE.PlaneGeometry(3.1,0.78), new THREE.MeshStandardMaterial({map:texPlaca(p[0],p[1],p[2]),roughness:.65})); pl.position.set(-9.3+i*3.1,0.46,GOLZ-2.1); scene.add(pl); }

    bola=new THREE.Mesh(new THREE.SphereGeometry(0.42,32,32), new THREE.MeshPhysicalMaterial({map:texBola(),roughness:.3,clearcoat:.55,clearcoatRoughness:.3}));
  ...
  resetChute(){
    if(camera){ camera.fov=46; camera.updateProjectionMatrix(); }
  ...
  comemorar(){
    ...
    this.tremor(0.4,540,2.4);   // punch-zoom dramático no gol
  },
  ...
  tremor(intens,dur,punch=0){
    if(!baseCamPos) return;
    if(punch>0){
      const alvo=baseCamPos.clone(); alvo.z-=punch; alvo.y+=punch*0.16; const f0=camera.fov;
      anima(150, k=>{ camera.position.lerpVectors(baseCamPos,alvo,easeOut(k)); camera.fov=f0-(f0-33)*easeOut(k); camera.updateProjectionMatrix(); camera.position.x+=(Math.random()-0.5)*intens*(1-k); camera.position.y+=(Math.random()-0.5)*intens*(1-k); }, ()=>{
        anima(dur, k=>{ camera.position.lerpVectors(alvo,baseCamPos,easeOut(k)); camera.fov=33+(f0-33)*easeOut(k); camera.updateProjectionMatrix(); }, ()=>{ camera.position.copy(baseCamPos); camera.fov=f0; camera.updateProjectionMatrix(); });
      });
    } else {
      anima(dur, k=>{ const d=(1-k)*intens; camera.position.set(baseCamPos.x+(Math.random()-0.5)*d, baseCamPos.y+(Math.random()-0.5)*d, baseCamPos.z); }, ()=>camera.position.copy(baseCamPos));
    }
  }
};
